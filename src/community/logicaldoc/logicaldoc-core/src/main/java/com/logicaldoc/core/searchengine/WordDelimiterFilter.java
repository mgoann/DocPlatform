...
final class WordDelimiterFilter extends TokenFilter {
	 ...
	 * @param catenateNumbers If 1, causes maximum runs of number parts to be
	 *        catenated: "500-42" => "50042"
	 ...
	 * @param preserveOriginal If 1, includes original words in subwords:
	 *        "500-42" => "500" "42" "500-42"
	 ...
	 *        delimited
	 */
	public WordDelimiterFilter(TokenStream in, byte[] charTypeTable, int generateWordParts, int generateNumberParts,
			int catenateWords, int catenateNumbers, int catenateAll, int splitOnCaseChange, int preserveOriginal,
			int splitOnNumerics, int stemEnglishPossessive, CharArraySet protWords) {
		super(in);
		...
		this.iterator = new WordDelimiterIterator(charTypeTable, splitOnCaseChange != 0, splitOnNumerics != 0,
	 ...
	 *        delimited
	 */
	public WordDelimiterFilter(TokenStream in, int generateWordParts, int generateNumberParts, int catenateWords,
			int catenateNumbers, int catenateAll, int splitOnCaseChange, int preserveOriginal, int splitOnNumerics,
			int stemEnglishPossessive, CharArraySet protWords) {
		this(in, WordDelimiterIterator.DEFAULT_WORD_DELIM_TABLE, generateWordParts, generateNumberParts, catenateWords,
				catenateNumbers, catenateAll, splitOnCaseChange, preserveOriginal, splitOnNumerics,
	...
	private void saveState() {
		...
		if (savedBuffer.length < termAttribute.length()) {
			savedBuffer = new char[getNextSize(termAttribute.length())];
		}
	...
	private static int getNextSize(int targetSize) {
		/*
